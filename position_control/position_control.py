...
kd = [0.]*4
ki = [0.]*4
controller = o80_pam.JointPositionController(robot_config,
                                             kp,kd,ki)
...
def _go_to(target_position):
    ...
    def _loop():
        ...
        while current_time-time_start < timeout:
            ...
            desired_pressures = controller.go_to(pressure_interface,
                                                 target_position,
                                                 current_time)
            pressure_interface.set(desired_pressures,
                                   burst=burst_mode)
            errors = _eval_error()
    ...
    if not finished:
        errors = _eval_error()
        print("failed to reach {} in {} seconds. errors: {}".format(_round(target_position),
                                                                    timeout,
                                                                    _round(errors)))
